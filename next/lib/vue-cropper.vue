  ...
  methods: {
        ...
        if (this.canChangeX) {
          if (this.changeCropTypeX === 1) {
            ...
            if (this.cropOldW + fw < minCropW) {
              ...
              // 右侧坐标抽 超过左侧
              this.cropW =
                wrapperW - this.cropChangeX + Math.abs(fw + this.cropOldW) <= wrapperW - minX ? Math.abs(fw + this.cropOldW) : this.cropChangeX - minX
              this.cropOffsertX =
                wrapperW - this.cropChangeX + Math.abs(fw + this.cropOldW) <= wrapperW - minX ? this.cropChangeX - Math.abs(fw + this.cropOldW) : minX
            }
          }
        }
        ...
        if (this.canChangeY) {
          if (this.changeCropTypeY === 1) {
            ...
            if (this.cropOldH + fh < minCropH) {
              ...
              this.cropH =
                wrapperH - this.cropChangeY + Math.abs(fh + this.cropOldH) <= wrapperH - minY ? Math.abs(fh + this.cropOldH) : this.cropChangeY - minY
              this.cropOffsertY =
                wrapperH - this.cropChangeY + Math.abs(fh + this.cropOldH) <= wrapperH - minY ? this.cropChangeY - Math.abs(fh + this.cropOldH) : minY
            }
          }
        }
    ...
    endCrop() {
      ...
      const { width, height } = this.fixed
        ? this.calculateSize(this.fixedNumber[0], this.fixedNumber[1], minCropW, minCropH, this.cropW, this.cropH)
